...
**Current release level: R0 — specification, verified synthetic UI, Compact compiler/runtime, native services and disposable wallet funding.** Node/indexer/prover and observed NIGHT/DUST funding now run without Docker. Milo deployment was rejected by the unchanged node's block limits; reservation did not run. Convex, Privy and Stripe remain unintegrated. Midnight completed provider/operation rows remain **0/6 and 0/14**. These bounded results are not R1, R3 or a live service.
...
| 2 — real local protocol; M-01–M-03, R1 | BLOCKED at deployment feasibility | Contract/platform | [Execution evidence](PROTOCOL_VERIFICATION.md); compiler artifacts, native services, finalized funding and observed DUST | Node rejects full deployment with RPC 1010, “Transaction would exhaust the block limits”; resolve within the canonical single-contract/admission policy, then prove/submit/observe and adversarial lifecycle |
...
| 2B — isolated local-network execution | BLOCKED at contract deployment, not infrastructure | Contract/platform | BLOCKED → ONGOING after native unblock → BLOCKED at node admission; [native lane](docs/native-network.md) verifies health, funding, DUST and cleanup | Do not raise chain limits or remove operations to manufacture success. Investigate bounded verifier installation before immutable admission; no deployment/reservation success |
| 2B.1 — native services and disposable funding checkpoint | DONE for this bounded slice | Contract/platform | [Sanitized receipt](docs/receipts/native-local-checkpoint.json); 37 Bun tests/397 assertions plus 16 separate Node tests | No whole provider/MID row closes; complete deployment, maintenance rejection and real operation/recovery evidence remain open |
| 3 — connected developer preview; M-04–M-06/B-03/B-04/B-07/B-09, R2 | PENDING | Backend/integration | No real account, file policy, browser-wallet integration or recovery evidence; isolated CLI funding is not connected-preview evidence | Named wallet/prover profile, isolated Convex/Privy configuration, independent actor recovery and authenticated files |
...
**Current checkpoint:** [published implementation `041d853`](https://github.com/amolikpe-bit/milo/commit/041d853c4db7c48c3c31c8d16232fe266c08a8d0) verifies native services, finalized disposable funding and observed DUST; its scoped service runs were stopped. Docker is no longer required for this local lane. Full contract deployment is blocked by the unchanged node's block limits (RPC 1010), not by Docker provisioning. The separate platform setup/reprovisioning limitations remain unresolved. **Blocked readiness:** R1 still requires successful deployment, admission/maintenance checks and actual protocol prove/submit/observe; R3 additionally requires configured provider environments, supported browser-wallet integration, payment/recovery evidence and the external approvals in stages 2–7. Organizer/network discrepancies remain unresolved.
