...
| 2 — real local protocol; M-01–M-03, R1 | ONGOING | Contract/platform | [Compiler/runtime evidence](PROTOCOL_VERIFICATION.md); original contract, complete keys/ZKIR, fixed vectors and negative controls verified | Docker/network configuration, real prove/submit/observe, admission/maintenance and real transaction races remain unverified; M-01 and dependent gates stay open |
...
| 2B — isolated local-network execution | PENDING | Contract/platform | Digest-pinned Compose candidate and source receipt only | Validate backing-service configuration, provision Docker, pull/boot/fund/reset, and prove/submit/observe; no network success yet |
