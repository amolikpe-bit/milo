...
export function changeRole(state: SimState, role: Role): SimState {
  ...
  return apply(
    ...
    { role, ready: false, filesVerified: false },
  );
}
  ...
  switch (name) {
    ...
    case "deploy":
      ...
      return apply(
        state,
        actionDescription(action),
        "Synthetic order moved to DEPLOYED; no chain transaction was sent.",
        { phase: "DEPLOYED" },
      );
    case "reserve":
      buyer();
    ...
    case "dispute":
      ...
      return apply(
        state,
        actionDescription(action),
        "Synthetic buyer dispute opened.",
        { phase: "DISPUTED" },
      );
...
function advanceDeadline(state: SimState): SimState {
  ...
  return apply(
    ...
    "Synthetic deadline advanced; no clock, network action, or automatic external effect occurred.",
    nextPhase,
  );
}
