...
| **1B — reproducible development environment** | Platform: M-01, B-01/B-02 feasibility | Same prototype can continue independently; no elaborate marketing polish | Selected Bun plus separate supported Node tooling; Compact devtools/compiler; Docker Engine + Compose v2 capable host; exact local-dev node/indexer/prover images; isolated testkit and synthetic funded actors | Frozen lockfile, exported API/type checks, generated-code build, container health, image digests, reset/funding and fresh-setup evidence. A local-dev smoke process that exits is not a persistent browser-test network |
...
| **Local Midnight — steps 1B–3** | Official local-dev currently requires Node `>=22`, Docker and Compose v2; use the selected separate Node pin. Default documented ports: node `9944`, indexer `8088`, proof server `6300`. Compact/compiler and exact artifact cohort are separate build requirements | Docker-capable machine/runner required; keep node/prover local or explicitly trusted and access-controlled. Synthetic actor stores and chain state must reset together; no real wallets. Do not expose default development funding identities or raw logs | Verify actual selected commit/images/digests, architecture compatibility, health, funding/DUST spendability and measured proof memory/disk/time. No universal RAM/CPU minimum is established by this receipt. Current workspace capability metadata reports Docker absent; provisioning remains future M-01 work [r4] |
