...
> **Planning baseline · revised 6 September 2026 · all delivery and adoption targets are prospective.**
...
4. [The accelerated delivery sprint](#4-the-accelerated-delivery-sprint)
...
**Day-0 administrative gate:** obtain written organizer confirmation of the applicable schedule, timezone, registration requirements, and whether the updated schedule supersedes the linked PDF. The PDF's Wave-1 deadline has already passed as of September 5. Do not represent Wave-1 eligibility as certain until the conflict is resolved. Continue safe development while seeking confirmation; do not delay engineering to wait for a reply.
...
The schedule below uses **D0–D10**. If D0 is September 5, D10 is September 15. Aim for an internal code/content freeze on September 14 and an earlier-than-deadline submission after the binding cutoff is confirmed. The final day is contingency, not an excuse to postpone contract integration. If the governing schedule differs, shift the submission plan and keep the engineering sequence.

That is the original staffing/sprint assumption, not completed work. At the September 6 revision, all implementation gates remain open. A later start requires explicit re-estimation and reduced optional scope—not moving the external deadline or pretending elapsed days produced evidence.
...
## 4. The accelerated delivery sprint

Day allocations are targets, not promises. Owners are roles; one person may cover more than one. Each day ends with an integrated, inspectable result and a short evidence entry.

| Day / provisional date | Protocol and integration | Product, API, and UX in parallel | Exit evidence and scope rule |
| --- | --- | --- | --- |
| **D0 · Sep 5** | Verify supported Midnight cohort; pin Bun/local Node boundary; compile and submit a minimal real contract | Confirm deadlines; freeze the separated ten-package budget; test Bun native dev/build/test and Convex deploy/auth/file feasibility; audit free UI source | B-01 plus bounded B-02–B-04 findings, real proving evidence, version/risk record. No elaborate hero before this passes |
| **D1 · Sep 6** | Implement canonical commitments, frozen roles, legal transitions, revisions, integer bounds | Convex schema, invite/membership/disabled-account model, Privy pairing, merchant offer/order wireframes, fixture reset | Allowed/forbidden runtime tests; buyer/merchant secrets independent; schema and custom-JWT review |
| **D2 · Sep 7** | Connect wallet, prove/submit/observe buyer and merchant actions; measure per-order cost/latency | Accessible order detail and timeline, explicit network/proof stages; generated React `useQuery` and Convex membership checks | Two separate actor profiles on a real local chain. If not achieved, cut optional visuals/sponsor work |
| **D3 · Sep 8** | Add trusted proving-path checks, artifact fingerprinting, stale/rejected transaction handling | Stripe authorization, Convex webhook inbox/outbox/reconciliation skeleton, authenticated private-file action | Contract state cannot be spoofed by client flags; payment holds and order states visibly separate |
| **D4 · Sep 9** | Complete deadlines, public-only timeout execution, operator dispute paths | Immutable delivery manifest, file verification, encrypted backup/import UX, wrong-network backup rejection | End-to-end local happy path plus dispute; clean-profile state restoration |
| **D5 · Sep 10** | Run Preview-network integration if supported; independent observer verifies state | Complete test capture/void; retry/crash recovery; order receipts; basic adapted landing live | Actual Preview and payment-test evidence, or explicit unmet gates. No mock promotion to “integrated” |
| **D6 · Sep 11** | Adversarial race/replay/time-boundary tests; role/terms leak review | Duplicate/out-of-order Convex inbox/webhook tests; authorization-expiry policy; operator queue; incident messages | Reproducible failures with safe outcomes; no unexplained double effects |
| **D7 · Sep 12** | Tune only measured proof/transaction bottlenecks | Browser Use Cloud synthetic-profile run with syntax-validated agent JSON/task script and structured output, reviewed axe native-DOM probe if capability is proven, CSS-first polish, keyboard/reduced-motion/mobile/manual accessibility tests; explore ThreeUI only after the relevant core gates pass | Provenance-backed cloud QA evidence or an explicit capability block; matching safe screenshots/video; usability issues ranked by blockage, not aesthetics |
| **D8 · Sep 13** | Independent security-oriented review; recovery and environment rebuild drill | Address user failures; complete private/public explanation, README, deck, limitations, source notices | Fresh environment reproduces flow; highest-risk findings fixed; no feature expansion |
| **D9 · Sep 14** | Release-candidate regression, pinned deployment manifest, exact commit/artifact recording | Capture final safe demo; dry-run pitch; verify all links and submission requirements | Internal freeze. R-level declared honestly; residual risks named |
| **D10 · Sep 15** | Critical fixes only with affected regression checks | Submit before the confirmed cutoff; preserve submitted SHA and media; prepare support follow-up | Submission evidence. Contingency for platform/upload failures, not new features |
...
Start with the highest-risk unproven assumption, not the most visible screen. Assign bounded parallel tasks with explicit interfaces. Integrate before the day ends. Update one short record: commit, environment, commands/results, media, unresolved risks, and next gate. Do not bury failures under a percentage-complete dashboard.
...
Use the provisional September 16–27 judging interval to:
...
**Provisional build window:** September 27–October 17, subject to the schedule resolution above.
...
**Provisional build window:** October 27–November 16, subject to confirmation.
