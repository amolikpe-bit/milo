...
> **Evidence checkpoint · 6 September 2026 · R0.** Read alongside the seven planning documents, not as implemented functionality. PR #9 was merged before this follow-up. No application code, dependency installation, wallet transaction or live service was added by this audit. Provider/operation evidence remains **0/6 and 0/14**.
...
This checkpoint reviewed the seven guides and both supplied program/rubric attachments; used the configured **Firecrawl MCP** for official-site discovery, searches and body retrieval; and cross-checked public repository, package and Maven evidence. Kapa is documented by Midnight but **was not a configured or queried MCP here**. Some Firecrawl bodies were same-day or earlier cached responses; the organizer API was fetched directly. No compiler, target network, Android device or EffectStream application was executed. This is a targeted source audit, not an exhaustive inspection of every organization repository, historical blog, transitive package or linked video. Earlier documentation's full-blog review is historical evidence, not a newly repeated crawl.
...
| EffectStream observer-only experiment | Could project admitted public chain fields, without touching actor secrets or becoming finality authority | **Conditional later.** Do not add a second observer merely to obtain a partner logo | Existing bounded observation must first show a concrete gap; then compare ordering, replay, restart/cursors, contradictory observations, logs and operating cost |
...
| DUST sponsorship | Native separation of action authority and fee payer reduces token-onboarding burden | **Prioritize after baseline, before decorative add-ons**, under existing B-10. Fee friction moves to sponsor capacity/cost; it does not disappear | Admitted payload/expiry, tamper/replay/ambiguity, depleted capacity, actual per-action funding and no silent user-paid fallback [sponsor] |
...
**EffectStream:** the inspected V2 template joins public EVM ERC-721 ownership and deliberately disclosed Midnight properties by a shared key. Its README expressly distinguishes that projection from atomic cross-chain execution. Current code is Bun-based and uses **Effection**, not the similarly named Effect library. The selected local template includes Hardhat, Midnight node/indexer/prover, sync node, batcher and frontend, with database responsibilities as well. Its sample raw decoded-state logging must never receive Milo's protected terms, witnesses or delivery bytes. Do not describe all template components as necessarily required for every possible observer-only use; measure the selected subset.

**Kuira:** `dapp-ui:0.1.0-alpha05` is one direct Gradle declaration exposing a multi-module graph, not a one-dependency system. The public repository says production use is not recommended and the SDK source repository remains private, while Kotlin sources and binaries are distributed through Maven. Local proving uses native code; a connector WebView is not itself the prover. This is a credible candidate, but published source artifacts do not establish reproducible upstream native builds, wallet security or compatibility with Milo. The README's root integration/security links require following the actual `docs/` files.
...
The API's active-wave deadline is **2026-09-16 15:00 UTC = September 17 00:00 JST**. The [Official Rules PDF][rules] still lists a September 2 Wave-1 end and contains conflicting overall dates/originality wording. The live page permits materially new Midnight work on existing code; the PDF also says net-new from August 5 while retaining existing-license language. The API is evidence of platform configuration, not a legal ruling that supersedes the PDF. Written organizer clarification remains necessary.
...
| 4 — B-09/B-10/B-11 and UC-13 | Prove reuse, wallet comprehension and then bounded sponsorship | Existing free-tier/asset/account-budget canaries first; no mainnet funding, paid plans or automatic top-up | Same machinery supports UC-13; measured proof/fee/onboarding/recovery behavior; no silent alternative recipient or payer |
...
Retrieved/reviewed **6 September 2026**. Repository pins identify inspected source; SaaS/docs observations are unversioned and may change. No listed version is a newly installed dependency.

| Source | Dated/pinned observation | What it establishes, not a canary claim |
| --- | --- | --- |
| [Official support matrix][matrix] | Current page: compiler `0.31.1`, runtime `0.16.0`, Midnight.js/testkit `4.1.1`, connector `4.0.1`, Wallet SDK `1.2.0`; Preview node `1.0.1`, indexer `4.3.5`, prover `8.1.0` | Matches the selected blueprint cohort. Other generated API/compatibility pages and the August node announcement differ; target endpoint execution remains necessary |
